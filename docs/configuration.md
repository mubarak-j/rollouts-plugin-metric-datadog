...
3. **`datadog` Secret in the controller namespace** — a Secret literally named `datadog` in the namespace the controller runs in. This is the Ibotta-standard Datadog secret; the plugin reads it automatically with no additional config.
